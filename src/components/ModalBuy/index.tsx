...
const ModalBuy: FC<IModalBuyProps> = ({ greetingUser, closeModal, modalCategory }) => {
  ...
  const [categoryDefault, setCategoryDefault] = useState<any>(copyModalCategory && copyModalCategory[0]);
  const [modalPrice, setModalPrice] = useState<number | undefined>(categoryDefault && categoryDefault.count);
  ...
  const changeSelectValue = (e: ChangeEvent<HTMLSelectElement>) => {
    let currentCategory = copyModalCategory?.filter((item) => item.descr === e.currentTarget.value)[0];
  ...
  function sendEmail(e: FormEvent<HTMLFormElement>) {
    ...
    if (phoneNumber.length === 13) {
      ...
      emailjs.sendForm('service_zwzcuj8', 'template_nnofpol', element, 'user_lMzDOCxB7FMfmgkx2Qr4C').then();
  ...
  return (
    <form className={styles['contact-form']} onSubmit={sendEmail}>
      ...
      />
      <PhoneInput
        className={`${styles['contact-form__phone']} ${errorNumber ? styles['contact-form__phone--error'] : ''}`}
        ...
        <>
          ...
          <div className={styles['contact-form__counter']}>
            <button
              ...
              type={'button'}
            />
            <input className={styles['contact-form__count']} id="count" name="count" readOnly value={countProduct} />
            <button
      ...
      <input type="text" id="totalPrice" name="totalPrice" value={modalPrice} style={{ display: 'none' }} />
      <textarea
